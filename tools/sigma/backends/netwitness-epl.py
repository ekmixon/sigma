    ...
    def generateMapItemNode(self, node):
        ...
        if self.mapListsSpecialHandling == False and type(value) in (str, int, list) or self.mapListsSpecialHandling == True and type(value) in (str, int):
            if type(value) == str and "*" in value[1:-1]:
                ...
                value = re.sub('\\*', '.*', value)
                value = re.sub('\\?', '.', value)
                return f"({key} REGEXP {self.generateValueNode(value)})"
            ...
            if not (regex.endswith('$') or regex.endswith('.*')):
                regex += '.*'
            return f"({key} REGEXP {self.generateValueNode(regex)})"
        else:
            raise TypeError(
    ...
    def generateMapItemListNode(self, key, value):
        ...
        if equallist:
            if len(equallist) == 1:
                ...
                fmtitems.append(f'{key} IN {", ".join(equallist)}')

        if containlist:
            fmtitems.append(f'{key} LIKE {f" OR {key} LIKE ".join(containlist)}')
        if regexlist:
            fmtitems.append(f'{key} REGEXP {"|".join(regexlist)}')
